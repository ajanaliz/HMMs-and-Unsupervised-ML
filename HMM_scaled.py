...
class HMM:
    ...
    def fit(self, X, max_iter=30):
        ...
        for it in xrange(max_iter):
            ...
            betas = []
            scales = []
			#we're going to have as many scales as we have alphas and betas
            #we can store logP instead of P
			logP = np.zeros(N)
            for n in xrange(N):
                x = X[n] # nth observation is stored in x.
                T = len(x) # length of the nth observation.
                scale = np.zeros(T)
                # alpha1[n] = self.pi*self.B[:,x[0]]
                alpha = np.zeros((T, self.M))
                alpha[0] = self.pi*self.B[:,x[0]] # alpha prime.
                scale[0] = alpha[0].sum() # first scale.
                alpha[0] /= scale[0] # alpha hat.
                for t in xrange(1, T): # loop through all the other times.
                    alpha_t_prime = alpha[t-1].dot(self.A) * self.B[:, x[t]]
                    scale[t] = alpha_t_prime.sum()
                    alpha[t] = alpha_t_prime / scale[t] # alpha hat @ time t
                # P[n] = alpha[-1].sum()
                # print "alpha[-1].sum():", alpha[-1].sum()
                logP[n] = np.log(scale).sum() # calculate the log probability.
                alphas.append(alpha) # 
                scales.append(scale)
                ...
                beta[-1] = 1 # init step
                for t in xrange(T - 2, -1, -1): # induction step.
			...
			# since they dont depend on the probability anymore, we're going to update the numerators directly
            a_num = np.zeros((self.M, self.M))
            b_num = np.zeros((self.M, V))
            for n in xrange(N): # loop through all the samples
                x = X[n] # get the nth observation and its length
                T = len(x)
    ...
    def log_likelihood(self, x):
        ...
        scale[0] = alpha[0].sum()
        alpha[0] /= scale[0]
        for t in xrange(1, T):# induction steps.
            alpha_t_prime = alpha[t-1].dot(self.A) * self.B[:, x[t]]
            scale[t] = alpha_t_prime.sum()
			# rescale alpha at time t.
