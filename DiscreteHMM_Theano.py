...
class HMM:
	...
	#the only thing thats gonna change in the API is the fit function, and its going to include Theano.
    def fit(self, X, learning_rate=0.001, max_iter=10, V=None, p_cost=1.0, print_period=10):
        ...
        if V is None:# try to guess the vocabulary size if not passed in.
			#this is just in case the X that we pass in doesnt include the entire vocabulary
			V = max(max(x) for x in X) + 1
        N = len(X)
        print "number of train samples:", N
		
		# init the HMM variables.
	...
	# sets A,B and pi. 
    def set(self, pi, A, B):
		# first we need to create theano shared variables.
        ...
        # define cost
        thx = T.ivector('thx')
        def recurrence(t, old_a, x): # takes in time, the old value for alpha, and the sequence x
            a = old_a.dot(self.A) * self.B[:, x[t]]
            s = a.sum() # calculate scale.
            return (a / s), s

        [alpha, scale], _ = theano.scan(
            fn=recurrence,
            sequences=T.arange(1, thx.shape[0]), # the sequences will be the times which starts at 1.
            outputs_info=[self.pi*self.B[:,thx[0]], None],# initial value for alpha, and the none here means that the initial value for s will be none.
        ...
        self.cost_op = theano.function(
            inputs=[thx],
            outputs=cost,
            allow_input_downcast=True,# so it wont complain if we mix float32 and float64.
        )
        return thx, cost
