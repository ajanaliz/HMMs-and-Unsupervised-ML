...
class HMM:
    ...
    def fit(self, X, max_iter=30, eps=10e-1): # eps for smoothing
        ...
        self.mu = np.zeros((self.M, self.K, D)) # setting mu similar to how we do in k-means.
        for i in xrange(self.M):
            for k in xrange(self.K):
				# get a random index for every gaussian to choose a random sequence with.
                random_idx = np.random.choice(N)
                x = X[random_idx]
				# once we have our sequence, we pick a random time.
                random_time_idx = np.random.choice(len(x))
				# once we have our random time, we set mu of i,k to be whatever point is at that random time.
                self.mu[i,k] = x[random_time_idx]
		
        self.sigma = np.zeros((self.M, self.K, D, D))
        for j in xrange(self.M):
            for k in xrange(self.K):
				# set sigma to a circular gaussian.
        ...
        for it in xrange(max_iter):
            if it % 1 == 0:
                print "it:", it
			# create arrays for our alphas, betas, gammas and Bs.
            alphas = []
            betas = []
            gammas = []
            Bs = []
            # components = []
            P = np.zeros(N) # holds our sequence probabilities.
            ...
            for n in xrange(N):
                ...
                for j in xrange(self.M): # loop through every state.
                    for t in xrange(T): # loop through every time.
                        for k in xrange(self.K): # loop through every gaussian.
				...
				# calculate alpha and beta using the B we just calculated.
                ...
                beta[-1] = 1 # init value is 1.
                for t in xrange(T - 2, -1, -1): # loop through all the times.
                ...
                for t in xrange(T):
					# the denominator only depends on t, so we'll calculate that just now.
                    alphabeta = (alphas[n][t,:] * betas[n][t,:]).sum()
                    # print "alpabeta:", alphabeta
                    for j in xrange(self.M): # loop through the states.
						# calculate the alphabeta factor.
                        factor = alphas[n][t,j] * betas[n][t,j] / alphabeta
                        # mixture_j = component[j,:,t].sum()
                        for k in xrange(self.K): # loop through all the gaussians.
...
def fake_signal(init=simple_init):
	...
	# our HMM will have 2 states and 2 gaussians.
    hmm = HMM(2, 2)
    hmm.fit(signals)
